...
const Sidebar: React.FC = () => {
    ...
    return (
        <div className="bg-gray-100 border shadow-lg mx-2 text-black w-20 flex flex-col items-center py-4 min-h-auto rounded-lg">
            <div className="mb-8">
                <Wind size={32} />
            </div>
            <div className='w-full'>
                <ul className="w-full flex flex-col items-center justify-center m-0">
                    {
                        itemSideBar?.map((item, index) => (
                            <li className='w-full mb-5' key={index}>
                                <button onClick={() => handleSetPageActive(index + 1)} className={`${currentPageActive?.pageActive === (index + 1) ? "opacity-100 text-blue-600 font-semibold" : "opacity-50"} w-full flex flex-col items-center justify-center text-sm`}>
                                    {item?.icon}
                                    <p>{item?.title}</p>
                                </button>
                            </li>
